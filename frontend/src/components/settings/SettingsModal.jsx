...
export default function SettingsModal() {
  ...
  useEffect(() => {
    if (settingsOpen && !local)
      api.getSettings().then(s => { setLocal(s); setWorkspaceSettings(s); }).catch(() => {});
  }, [settingsOpen]);
      ...
      <div className="settings-modal wide">
        ...
        <div className="settings-layout">
          <div className="settings-nav">
            {["providers","editor","ai","git","ui"].map(t => (
              <button key={t} className={`settings-nav-item ${activeTab === t ? "active" : ""}`}
          ...
          <div className="settings-content">
            {activeTab === "providers" && (
              <div>
                <p className="settings-hint">API keys are set via <code>.env</code> on the server.</p>
                <div className="provider-list">
                  {Object.entries(providers).map(([id, p]) => (
                    <div key={id}
                      ...
                      {!p.configured && <p className="provider-unconfigured-msg">Set <code>{id.toUpperCase()}_API_KEY</code> in backend <code>.env</code></p>}
                    </div>
                  ))}
                </div>
              </div>
            )}
            {activeTab === "editor" && s.editor && (
              <div className="settings-section">
                <NumberInput label="Font Size" value={s.editor.fontSize} min={10} max={28} onChange={v=>patch("editor","fontSize",v)}/>
                <NumberInput label="Tab Size" value={s.editor.tabSize} min={2} max={8} onChange={v=>patch("editor","tabSize",v)}/>
                <SelectInput label="Word Wrap" value={s.editor.wordWrap}
                  options={[{value:"off",label:"Off"},{value:"on",label:"On"},{value:"wordWrapColumn",label:"At column"}]}
                  onChange={v=>patch("editor","wordWrap",v)}/>
                <SelectInput label="Line Numbers" value={s.editor.lineNumbers}
                  options={[{value:"on",label:"On"},{value:"off",label:"Off"},{value:"relative",label:"Relative"}]}
                  onChange={v=>patch("editor","lineNumbers",v)}/>
                <Toggle label="Show Minimap" value={s.editor.minimap} onChange={v=>patch("editor","minimap",v)}/>
            ...
            {activeTab === "ai" && s.ai && (
              <div className="settings-section">
                ...
                <NumberInput label="Context Lines per File" value={s.ai.contextLines} min={20} max={200} onChange={v=>patch("ai","contextLines",v)}/>
              </div>
            )}
            {activeTab === "git" && s.git && (
              <div className="settings-section">
